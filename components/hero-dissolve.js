...
// Shader from the supplied Drive reference; automatic playback replaces scroll progress.
...
window.dissolveHeroFrame = (video, reveal, reverse = false) => new Promise(resolve => {
  if(matchMedia('(prefers-reduced-motion: reduce)').matches || video.readyState<2){reveal();resolve();return;}
  let renderer, material, texture, geometry;
  const cleanup=()=>{renderer?.domElement.remove();geometry?.dispose();material?.dispose();texture?.dispose();renderer?.dispose();};
  try {
    renderer=new THREE.WebGLRenderer({alpha:true,antialias:false});
    renderer.setPixelRatio(1);renderer.setSize(innerWidth,innerHeight);
    const canvas=renderer.domElement;canvas.style.cssText='position:fixed;inset:0;width:100%;height:100%;z-index:310;pointer-events:none';canvas.setAttribute('aria-hidden','true');
    ...
    material=new THREE.ShaderMaterial({vertexShader:coverVertexShader,fragmentShader:coverFragmentShader,transparent:true,uniforms:{
      uTexture:{value:texture},uResolution:{value:new THREE.Vector2(innerWidth,innerHeight)},uImageResolution:{value:new THREE.Vector2(frame.width,frame.height)},
      uDissolve:{value:reverse?1:0},uCenter:{value:new THREE.Vector2(.5,.5)},uTime:{value:0},uGrayscale:{value:reverse?1:0},uEdgeIntensity:{value:reverse?.5:0},uEdgeBrightness:{value:reverse?0:1}
    }});
    geometry=new THREE.PlaneGeometry(2,2);scene.add(new THREE.Mesh(geometry,material));renderer.render(scene,camera);
    document.body.append(canvas);if(!reverse)reveal();
    const start=performance.now();
    const tick=now=>{
      try{
        const progress=Math.min(1,(now-start)/2100),p=reverse?1-progress:progress,u=material.uniforms;
        u.uDissolve.value=p;u.uGrayscale.value=Math.min(1,p/.4);u.uEdgeIntensity.value=p*.5;u.uEdgeBrightness.value=1-p;
        renderer.render(scene,camera);
        if(progress<1)requestAnimationFrame(tick);else{if(reverse)reveal();cleanup();resolve();}
      }catch{if(reverse)reveal();cleanup();resolve();}
    };
    requestAnimationFrame(tick);
  }catch{cleanup();reveal();resolve();}
});
